...
try {
  ...
  const uploaded = await api('/api/agent-avatars', 'POST', { mimeType: 'image/png', data: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=' });
  assert.equal(uploaded.status, 201, JSON.stringify(uploaded.data)); assert.match(uploaded.data.avatar, /^\/api\/agent-avatars\/[0-9a-f-]+\.png$/);
  const avatarResponse = await fetch(base + uploaded.data.avatar); assert.equal(avatarResponse.status, 200); assert.equal(avatarResponse.headers.get('content-type'), 'image/png');
  const bad = await api('/api/agents/validate', 'POST', { id: 'Bad ID' }); assert.equal(bad.status, 400); assert.ok(bad.data.fieldErrors.id);
  assert.equal((await api('/api/agents/planner')).data.avatar, '🧭');
  const input = { id: 'qa-reviewer', name: 'QA Reviewer', description: '检查实现结果', capabilities: ['review'], systemPrompt: '请严格检查结果并给出结论。', model: 'mock:reviewer', tools: ['fs.read'], disallowedTools: [], permissionMode: 'readonly', color: '#3366aa', avatar: '🧪' };
  const created = await api('/api/agents', 'POST', input); assert.equal(created.status, 201); assert.equal(created.data.version, 1); assert.equal(created.data.avatar, '🧪');
  const badAvatar = await api('/api/agents/validate', 'POST', { ...input, avatar: 'http://example.com/avatar.png' }); assert.equal(badAvatar.status, 400); assert.ok(badAvatar.data.fieldErrors.avatar);
  ...
  const updated = await api('/api/agents/qa-reviewer', 'PATCH', { ...input, avatar: uploaded.data.avatar, description: '更新后的描述', expectedVersion: 1 }); assert.equal(updated.status, 200); assert.equal(updated.data.version, 2); assert.equal(updated.data.avatar, uploaded.data.avatar);
  ...
  const snapshot = JSON.parse(db.prepare('SELECT definition FROM run_agent_snapshots WHERE run_id=?').get(room.data.run.id).definition); db.close(); assert.equal(snapshot.version, 2); assert.equal(snapshot.description, '更新后的描述'); assert.equal(snapshot.avatar, uploaded.data.avatar);
