...
新版 Contract 已使用类型化 [Successor Obligation](./runtime-successor-obligations.md) 取代子 Subject/Dispatch 字符串推断，并使用 [EvidenceBundle](./runtime-evidence-context-loop-guard.md) 冻结 EvidenceRef 解析结果和内容哈希；历史 Contract 继续保留原派生投影和直接 EvidenceRef 校验路径。
