    ...
    <aside className="flex w-80 shrink-0 flex-col border-l border-zinc-800 bg-zinc-900/60">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {tab === 'collaboration' && <div className="space-y-3 text-xs">
          ...
          {activeCandidates.length > 0 && <section className="space-y-2"><div className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">完成候选</div>{activeCandidates.slice().reverse().map((candidate) => <details key={candidate.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-2"><summary className="cursor-pointer list-none"><div className="flex justify-between gap-2"><span className="text-zinc-300">{candidate.agentId} · generation {candidate.generation}</span><span className={candidate.status === 'accepted' ? 'text-emerald-300' : candidate.status === 'rejected' ? 'text-red-300' : candidate.status === 'superseded' ? 'text-amber-300' : 'text-sky-300'}>{candidate.status}</span></div></summary><div className="mt-2 space-y-1 border-t border-zinc-800 pt-2 text-[11px]"><div className="text-zinc-600">Subject {candidate.subjectKey}</div><pre className="max-h-32 overflow-auto whitespace-pre-wrap rounded bg-zinc-950/70 p-2 text-zinc-400">{candidate.summary}</pre>{candidate.reasons.length > 0 && <div className="text-amber-300">{candidate.reasons.join(' · ')}</div>}{candidate.feedback && <div className="rounded bg-red-500/10 p-2 text-red-300">{candidate.feedback}</div>}</div></details>)}</section>}
          {activeObligations.length > 0 && <section className="space-y-2"><div className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">后继义务</div>{activeObligations.slice().reverse().map((obligation) => <details key={obligation.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-2"><summary className="cursor-pointer list-none"><div className="flex justify-between gap-2"><span className="text-zinc-300">{obligation.kind} · generation {obligation.generation}</span><span className={obligation.status === 'satisfied' ? 'text-emerald-300' : obligation.status === 'open' ? 'text-sky-300' : obligation.status === 'failed' ? 'text-red-300' : 'text-amber-300'}>{obligation.status}</span></div></summary><div className="mt-2 space-y-1 border-t border-zinc-800 pt-2 text-[11px] text-zinc-500"><div>stable key：{obligation.stableKey}</div><div>parent：{obligation.parentSubjectId.slice(0, 8)}{obligation.targetSubjectId ? ` · target：${obligation.targetSubjectId.slice(0, 8)}` : ''}</div>{obligation.resolutionSourceId && <div>resolved by：{obligation.resolutionSourceId}</div>}</div></details>)}</section>}
          {activeEvidenceBundles.length > 0 && <section className="space-y-2"><div className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">证据包</div>{activeEvidenceBundles.slice().reverse().map((bundle) => <details key={bundle.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-2"><summary className="cursor-pointer list-none"><div className="flex justify-between gap-2"><span className="text-zinc-300">{bundle.ownerType} · {bundle.refs.length} 项</span><span className={bundle.status === 'valid' ? 'text-emerald-300' : bundle.status === 'drifted' ? 'text-red-300' : 'text-amber-300'}>{bundle.status}</span></div></summary><div className="mt-2 space-y-1 border-t border-zinc-800 pt-2 text-[11px] text-zinc-500"><div>fingerprint：{bundle.fingerprint.slice(0, 16)}</div><div>owner：{bundle.ownerId}</div><div>校验时间：{bundle.validatedAt}</div></div></details>)}</section>}
          {activeRouteGuards.length > 0 && <section className="space-y-2"><div className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">路由防循环</div>{activeRouteGuards.slice().reverse().map((event) => <div key={event.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-2 text-[11px]"><div className="flex justify-between gap-2"><span className="text-zinc-300">{event.fromAgentId} → {event.targetAgentId} · {event.repeatedCount}</span><span className={event.outcome === 'blocked' ? 'text-red-300' : event.outcome === 'warned' ? 'text-amber-300' : 'text-emerald-300'}>{event.outcome}</span></div>{event.reason && <div className="mt-1 text-zinc-500">{event.reason}</div>}</div>)}</section>}
