...
import type { AgentCapability, AgentDefinition, AgentInput, AgentOptions } from '@agent-gand/shared';
...
export function AgentManager() {
  ...
  useEffect(() => { void Promise.all([refresh(), api.getAgentOptions().then(setOptions)]); }, []);
  ...
  return <div className="h-full overflow-y-auto p-4">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{sorted.map((agent) => <article key={agent.id} className={`rounded-xl border p-4 ${agent.enabled ? 'border-zinc-800 bg-zinc-900/70' : 'border-zinc-900 bg-zinc-950 opacity-60'}`}>
      ...
      <div className="mt-3 flex flex-wrap gap-1">{agent.capabilities.map((cap) => <span key={cap} className="rounded-full bg-violet-500/10 px-2 py-1 text-[11px] text-violet-300">{capabilityLabel[cap]}</span>)}<span className="rounded-full bg-zinc-800 px-2 py-1 text-[11px] text-zinc-400">{agent.model}</span></div>
    ...
    {form && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setForm(null); }}><div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-950 p-5 shadow-2xl">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <Field label="模型"><input value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} placeholder="anthropic:claude-..." className="input" />{options && <p className="mt-1 text-[11px] text-zinc-600">{options.providers.map((p) => `${p.label}${p.configured ? ' ✓' : '（未配置）'}`).join(' · ')}</p>}</Field>
        ...
        <Field label="允许工具"><div className="flex flex-wrap gap-2">{options?.tools.map((tool) => <Check key={tool.name} checked={form.tools.includes(tool.name)} label={tool.name} onChange={() => setForm({ ...form, tools: form.tools.includes(tool.name) ? form.tools.filter((x) => x !== tool.name) : [...form.tools, tool.name], disallowedTools: form.disallowedTools.filter((x) => x !== tool.name) })}/>)}</div></Field>
        <Field label="明确禁用工具"><div className="flex flex-wrap gap-2">{options?.tools.map((tool) => <Check key={tool.name} checked={form.disallowedTools.includes(tool.name)} label={tool.name} onChange={() => setForm({ ...form, disallowedTools: form.disallowedTools.includes(tool.name) ? form.disallowedTools.filter((x) => x !== tool.name) : [...form.disallowedTools, tool.name], tools: form.tools.filter((x) => x !== tool.name) })}/>)}</div></Field>
