...
const definitions: CoordinationProtocolDefinition[] = [
  {
    ...
    composable: false, inputTypes: ['objective'], outputTypes: ['result'], allowedSuccessors: [], runtimeMode: 'collaboration', risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'result'], outputTypes: ['result'], allowedSuccessors: ['sequential_pipeline', 'parallel_fanout', 'review_revision', 'supervisor_dag'], runtimeMode: 'pipeline', risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'result'], outputTypes: ['branches'], allowedSuccessors: ['supervisor_aggregation', 'review_revision', 'consensus', 'vote'], runtimeMode: 'pipeline', risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['branches'], outputTypes: ['result'], allowedSuccessors: ['sequential_pipeline', 'review_revision', 'supervisor_dag'], runtimeMode: 'pipeline', risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'result'], outputTypes: ['result'], allowedSuccessors: ['review_revision'], runtimeMode: 'supervisor', risk: 'medium',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'result', 'branches'], outputTypes: ['reviewed_result'], allowedSuccessors: [], runtimeMode: 'pipeline', risk: 'medium',
  },
  {
    ...
    composable: true, inputTypes: ['objective'], outputTypes: ['adjudicated_result'], allowedSuccessors: [], runtimeMode: 'pipeline', risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'branches'], outputTypes: ['consensus_result'], allowedSuccessors: ['review_revision'], runtimeMode: null, risk: 'low',
  },
  {
    ...
    composable: true, inputTypes: ['objective', 'branches'], outputTypes: ['decision'], allowedSuccessors: ['review_revision'], runtimeMode: null, risk: 'low',
  },
  {
    ...
    composable: false, inputTypes: ['objective'], outputTypes: ['result'], allowedSuccessors: [], runtimeMode: 'collaboration', risk: 'medium',
  },
];
...
function clone(definition: CoordinationProtocolDefinition): CoordinationProtocolDefinition {
  return {
    ...
    inputTypes: [...definition.inputTypes], outputTypes: [...definition.outputTypes], allowedSuccessors: [...definition.allowedSuccessors],
...
export function validateProtocolComposition(ids: CoordinationProtocolId[]): string[] {
  const issues: string[] = [];
  const first = definitions.find((item) => item.id === ids[0]);
  if (first && !first.inputTypes.includes('objective')) issues.push(`PROTOCOL_INITIAL_INPUT_MISMATCH:${first.id}`);
  for (let index = 0; index < ids.length - 1; index += 1) {
    const current = definitions.find((item) => item.id === ids[index]);
    const next = definitions.find((item) => item.id === ids[index + 1]);
    if (!current || !next) continue;
    if (!current.composable || !next.composable) issues.push(`PROTOCOL_NOT_COMPOSABLE:${!current.composable ? current.id : next.id}`);
    if (!current.allowedSuccessors.includes(next.id)) issues.push(`PROTOCOL_CONNECTION_NOT_ALLOWED:${current.id}->${next.id}`);
    if (!current.outputTypes.some((type) => next.inputTypes.includes(type))) issues.push(`PROTOCOL_TYPE_MISMATCH:${current.id}->${next.id}`);
  }
