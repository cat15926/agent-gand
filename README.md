...
- **沙箱工作区（§9/§10）**：无前缀路径 = 当前工作区（默认每次 run 独立 `sandbox/runs/<runId>/`，或 `POST /api/runs` 指定命名工作区 `sandbox/workspaces/<name>/` 跨 run 复用）；`shared/` = 团队共享区（写入强制人工审批）；`archive/` = 根级历史归档只读。并发写同一命名工作区在 MVP 下接受（单用户场景），不设锁。外部工作区（`ext:`）对 Coordination run 按计划隔离：无前缀路径映射到 `<extRoot>/<planId 前 8 位>/` 子目录，避免并发 run 产物混写与裁判误读。外部目录默认写入逐次审批；注册时可标记**信任**（或列表中切换），信任目录内 `fs.write` 免审批，隔离与包含性检查不变。
